...
- Oracle fixtures on cone, sphere and torus faces (`sweep/revolve-frustum`, `revolve-barrel`, `revolve-ring`) once C2's intersector gives S5 its plane–cone, plane–sphere, plane–torus and cylinder–torus arms and B1 a ray against them; until then the quadric-faced revolves are closed-form tests with the oracle reading their STEP through `oracle::scratch_fixture` (M5 `⚠ OPEN` 2)

...
- [x] Step 7 — Fixture corpus. `tests/fixtures/README.md`; a
...
- Step 7: the corpus lint lives in `crates/arris/tests/corpus_lint.rs`
  as assumed; the lint logic itself is `arris_debug::fixtures::lint` so
  the test can run it on a scratch copy. The Rust side has its own
  expression evaluator (`arris_debug::fixtures::expr`) because `analytic`
  values are written as closed forms (`"pi * 16 * 12"`) and M5's recipe
  runner will need it anyway. The recipe hash is defined as serde_json's

...
| `arris-ops` | Primitives, extrude and revolve of a `Profile`, transform, booleans, the blends, each returning `Provenance`; the queries `measure` (mass properties) and `query` (projection onto a plane) | `arris-check`, `thiserror`, `rayon` (feature) | 2 — algorithms |

...
|---|---|---|
| Is it valid? | `arris-check` — `arris_check::check(&model, body, Level::Fast)` (rows M, V, E run; loops, faces, shells and bodies follow in M2 steps 7–8) | Every violated invariant with the entity that violates it, one line each, sorted by entity. Read this first; most "wrong picture" bugs are a checker line. |

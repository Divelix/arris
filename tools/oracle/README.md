...
| `occt_assembly.py <fixture-a> <fixture-b> <out.step>` | Builds both recipes, makes an XCAF assembly of the first placed once and the second placed twice (each a turn and a shift), writes it with `STEPCAFControl_Writer` and prints each placed instance's volume and centroid as JSON: what Arris's reader of an assembly is held to (ADR-0025 §5). `arris_debug::oracle::occt_assembly` is the Rust seam, cached under both `fixture.json`s |
